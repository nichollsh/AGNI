...
module atmosphere
    ...
    **Set parameters of the atmosphere.**
    ...
    function setup!(atmos::atmosphere.Atmos_t,
        ...
        #    radii
        atmos.r             = ones(Float64, atmos.nlev_c) # radii at cell centres [m]
        atmos.rl            = ones(Float64, atmos.nlev_l) # radii at cell edges [m]
        atmos.layer_thick   = ones(Float64, atmos.nlev_c)
        #    gravity
        atmos.g             = ones(Float64, atmos.nlev_c) * atmos.grav_surf
        atmos.gl            = ones(Float64, atmos.nlev_l) * atmos.grav_surf
        #    net acceleration
        atmos.a             = ones(Float64, atmos.nlev_c)
        atmos.al            = ones(Float64, atmos.nlev_l)
        #    enclosed mass [kg]
        ...
        atmos.layer_σ       = ones(Float64, atmos.nlev_c)
        atmos.layer_isbound = trues(atmos.nlev_c)

        # Initialise thermodynamic properties
        atmos.layer_μ       = ones(Float64, atmos.nlev_c) # default safe value
        atmos.layer_ρ       = ones(Float64, atmos.nlev_c)
        atmos.layer_cp      = ones(Float64, atmos.nlev_c)
        atmos.layer_kc      = ones(Float64, atmos.nlev_c)
        atmos.layer_Hp      = ones(Float64, atmos.nlev_c)
