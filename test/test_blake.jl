...
include(joinpath(ROOT_DIR, "src", "util", "blake.jl")); import .blake
...
@testset "blake" begin
    ...
    hash_result = blake.hash_file(test_file)
    ...
    hash_result = blake.hash_file(temp_file; quiet=true)
    ...
    validity = blake.valid_file(temp_file; quiet=true)
    ...
    write(temp_file, "test content")
    correct_hash = blake.hash_file(temp_file; quiet=true)
    write(temp_file * ".chk", correct_hash)
    validity2 = blake.valid_file(temp_file; quiet=true)
    @test validity2 == true
    rm(temp_file, force=true)
