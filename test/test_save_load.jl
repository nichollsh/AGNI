...
@testset "save_load" begin
    ...
    @testset "load_ncdf" begin
        ...
        @test atmos2.tmp_surf ≈ atmos.tmp_surf
        @test atmos2.tmp_magma ≈ atmos.tmp_magma
        @test atmos2.skin_d ≈ atmos.skin_d
        @test atmos2.skin_k ≈ atmos.skin_k
        @test atmos2.rp ≈ atmos.rp
        @test atmos2.grav_surf ≈ atmos.grav_surf
        @test atmos2.instellation ≈ atmos.instellation

        # Check array values
        @test all(atmos2.p .≈ atmos.p)
        @test all(atmos2.pl .≈ atmos.pl)
        @test all(atmos2.tmp .≈ atmos.tmp)
        @test all(atmos2.tmpl .≈ atmos.tmpl)

        # Check gas VMRs
        for gas in atmos.gas_names
            @test all(atmos2.gas_vmr[gas] .≈ atmos.gas_vmr[gas])
        end

        # Check flux values
        @test all(atmos2.flux_u .≈ atmos.flux_u)
        @test all(atmos2.flux_d .≈ atmos.flux_d)
        @test all(atmos2.flux_n .≈ atmos.flux_n)
        @test all(atmos2.flux_cdry .≈ atmos.flux_cdry)
